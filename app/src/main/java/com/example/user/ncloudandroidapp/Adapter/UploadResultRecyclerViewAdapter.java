...
public class UploadResultRecyclerViewAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {

    private static final String TAG = "UploadResultRecyclerViewAdapter";
    ...
    public UploadResultRecyclerViewAdapter(Context context) {
    ...
    public void addLocalItem(LocalGalleryItem item){
        itemObjects.add((Item)item);
        notifyItemInserted(itemObjects.size() - 1);
    }

    public void addAllLocalGalleryItems(List<LocalGalleryItem> items){
        for(LocalGalleryItem item : items){
            addLocalItem(item);
        }
    }
